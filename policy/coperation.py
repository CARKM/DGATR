...
class DRL(Policy):
    ...
    def __init__(self, network, net_name='6x6',
        ...
        self.adj, _, _ = get_adj_mtx(self.links.items(),
    ...
    def reset_optimizer(self, learning_rate):
        self.optimizer = {i: torch.optim.Adam(self.eval_net[i].parameters(),
                                              lr=learning_rate) 
    ...
    def build_model(self, NN_eval, NN_target):
        ...
        self.optimizer = {i: torch.optim.Adam(self.eval_net[i].parameters(),
                                              lr=self.config.learning_rate) for i in range(node)}
    ...
    def _update_parameter(self, x, s, a, r, s_, a_, is_dest):
        ...
        ava_act_ = s_[:, 1::self.config.node_input]
        ...
        # update local neural network
        self.optimizer[x].zero_grad()
        loss.backward()
        self.optimizer[x].step()

        # communicate with neighbour network
        if self.learn_step_counter[x] % 1 == 0:
            for param in self.eval_net[x].parameters():
                param.data.copy_(param.data * self.adj[x, x])
            for i in range(self.config.number_of_node):
                if self.adj[x, i] and x != i:
                    for param_x, param_i in zip(self.eval_net[x].parameters(), self.eval_net[i].parameters()):
                        param_x.data.copy_(param_x.data + param_i.data * self.adj[x, i])
    ...
    def store(self, filelist):
        for x in range(self.config.number_of_node):
            torch.save(self.eval_net[x].state_dict(), filelist[x])
